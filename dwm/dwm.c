...
static void resizefloating(const Arg *arg);
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - statusw - getsystraywidth() - 2 * sidepad) {
			x = selmon->ww - statusw - getsystraywidth() - 2 * sidepad ;
			click = ClkStatusText;
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, barpad, w, bh - barpad * 2, 0, text, 0);
...
monocle(Monitor *m)
{
	...
	if (n > 0) /* override layout symbol */
		...
		snprintf(m->ltsymbol, sizeof m->ltsymbol, "󰬔");
...
tilewide(Monitor *m)
{
  ...
  for (i = mx = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
      ...
      resize(c, m->wx + mx, m->wy, w - (2*c->bw) + gappx - (m->nmaster >= n ? gappx : 0 ), (m->wh - ty) - (2*c->bw), 0);
      if  (mx + WIDTH(c) < m->ww)
        mx += WIDTH(c);
    } else {
      h = (m->wh - ty) / (n - i);
